...
class CriticNetwork():
    ...
    def copy_weights2(self,sess,network,tau):
        sess.run([self.w1.assign((1-tau)*self.w1 + tau*network.w1),
                  ...
                  self.actionw1.assign((1-tau)*self.actionw1 + tau*network.actionw1)])
    def copy_weights(self,sess,network,tau):
        # sess.run(w1.assign(20)) megvaltoztatna
        ...
        self.actionw1 = tf.Variable(sess.run((1-tau)*self.actionw1 + tau*network.actionw1))
        self.l2 = tf.nn.relu(tf.matmul(self.l1, self.w2) +
                             tf.matmul(self.action_in,self.actionw1) +
                             self.b2);
        self.l3 = tf.identity(tf.matmul(self.l2, self.w3) + self.b3)
